...
public class MargemBruta implements IndiceBalanco<BalancoPatrimonial, Double> {

    @Override
    public String nome() {
        return IndiceBalancoEnum.MARGEM_BRUTA.name();
    }

    @Override
    public Function<BalancoPatrimonial, Double> formula() {
        return (balanco) -> {
            Optional<ItemBalanco> lucroBruto = balanco.buscaItem(ItemBalancoEnum.LUCRO_BRUTO);
            Optional<ItemBalanco> vendasLiquidas = balanco.buscaItem(ItemBalancoEnum.VENDAS_LIQUIDAS);
            
            if(lucroBruto.isPresent() && vendasLiquidas.isPresent()) {
                Double v1 = lucroBruto.get().valorContabil();
